...
export default class PageNavBar extends Component {
    ...
    render() {
        ...
        if (pageNums) {            
            if (pageNums > 4){   //非初始化
                ...
                for (var i = 1; i <= pageNums; i++) {
                    ((i)=>{
                    items.push( < li key = { i } > < a name="pagenum" id={'pagenum'+(i)} value={i} onClick={(e)=>this.props.pagego(e,i)}> { i } < /a></li > );
                    })(i)
                };
        ...
        return ( < ul className = "pagedown" >
            < li > < button className = "btn-primary" onClick={this.props.firstpage}> 首页 < /button></li >
            < li > < button className = "btn-primary" onClick={this.props.pageup}> 上一页 < span className = "caret" style = {{ transform: "rotate(180deg)" } } > < /span></button > < /li> 
            { items } 
            < li > < button className = "btn-primary" onClick={(e)=>this.props.pagedown(e,pageNums)}> 下一页 < span className = "caret" > < /span></button > < /li> 
            < li > < button className = "btn-primary" onClick={(e)=>this.props.lastpage(e,pageNums)}> 尾页 < /button></li >
            < /ul>
        )
    }
}
