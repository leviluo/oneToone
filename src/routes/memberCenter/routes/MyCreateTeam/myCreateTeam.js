        ...
        <div className="createTeam">
            ...
            {this.state.OrganizationByMe.map((item,index)=>{
              ...
              return <div className="items" key = {index}>
                      {!this.state[item.name] && <div>{item.name}<span><Link to={linkApproval} >{item.requests > 0 && <span className="noRead">{item.requests}</span>}&nbsp;入社申请</Link><Link to={link} >去社团主页</Link><a onClick={(e)=>{this.state[item.name] = true;this.setState({})}}><i className="fa fa-edit"></i>修改</a><a onClick={(e)=>this.deleteOrganization(e,item.id)}><i className="fa fa-trash"></i>解散</a></span></div>}
