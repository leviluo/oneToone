...
const publicController = {
    ...
    items:async function(next){
        ...
        if (this.request.body.parentSpeciality) {
           var result = await sqlStr("select m.nickname,m.address,m.sex,m.phone,ms.memberId,s.name,ms.brief from member as m left join memberSpeciality as ms on ms.memberId = m.id left join specialities as s on s.id = ms.specialitiesId where s.categoryId = (select id from specialityCategory where name = ? ) and (m.location = ? or m.location = ?) limit "+this.request.body.limit,[this.request.body.parentSpeciality,this.request.body.address,this.request.body.address+'市'])
           ...
           var result = await sqlStr("select m.nickname,m.address,m.sex,m.phone,ms.memberId,s.name,ms.brief from member as m left join memberSpeciality as ms on ms.memberId = m.id left join specialities as s on s.id = ms.specialitiesId where s.name = ? and (m.location = ? or m.location = ?) limit "+this.request.body.limit,[this.request.body.speciality,this.request.body.address,this.request.body.address+'市'])
           var count = await sqlStr("select count(m.id) as count from member as m left join memberSpeciality as ms on ms.memberId = m.id left join specialities as s on s.id = ms.specialitiesId where s.name = ? and (m.location = ? or m.location = ?)",[this.request.body.speciality,this.request.body.address,this.request.body.address+'市'])
        }
